...
for file_list = 1:length(listing_cell)
    ...
    %% filters %%
    % set threshold for short lines (at least median length)
    shortest_len_tokeep = round(nanmedian(len));
    shortest_len_tokeep_nocb = round(nanmedian(len_nocb));
    ...
    for kkk = 1:length(x_line)
        ...
        temp = intensity_linescan(kkk).int;
        if ~any(isnan(temp)) == 1                   % check if there are holes along the line (in case: discard)
            if length(temp) > shortest_len_tokeep   % remove short linescans
                temp(end+1:max_len, 1) = NaN;
                intensity_linescan_matrix_long(:,count) = temp;
                count = count + 1;
            end
        end
        
        temp_nocb = intensity_linescan_nocb(kkk).int;
        if ~any(isnan(temp_nocb)) == 1
            if length(temp_nocb) > shortest_len_tokeep_nocb
                temp_nocb(end+1:max_len_nocb, 1) = NaN;
                intensity_linescan_matrix_nocb_long(:,count_nocb) = temp_nocb;
                count_nocb = count_nocb + 1;
            end
        end
        
    end
    
    av_int = nanmean(intensity_linescan_matrix_long, 2);
    av_int_nocb = nanmean(intensity_linescan_matrix_nocb_long, 2);
    
    clear intensity_linescan_matrix_long intensity_linescan_matrix_nocb_long
    
    %% save %%
    
    save(fullfile(directory_cb, 'filter_av_int_withcb.mat'), 'av_int');
    save(fullfile(directory_cb, 'filter_av_int_withoutcb.mat'), 'av_int_nocb');
